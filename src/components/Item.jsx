...
function Item({ text, checked, id, edit, remove, onCheked, date }) {

  const handleOnCheked = (e) => {
    const complited = e.target.checked;
    onCheked(id, complited)
  }
...
return (
    <li className="todo__item">
      <div className="todo__item-check">
        <input checked={checked || false}  type="checkbox" id={`checkbox__${id}`} onChange={handleOnCheked} />
      ...
      <span className="todo__item-text">{text}</span>
      <div>Date added: {date}</div>
      <div className="todo__item-actions">
        <div className="todo__item-actions-edit" onClick={() => edit(id, text)} >
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
        ...
        <div className="todo__item-actions-remove" onClick={() =>  remove(id)}>
          <svg
            width="11"
            height="11"
            viewBox="0 0 11 11"
